...
public class PassFlowServiceImpl implements PassFlowService{
	...
	public int addConsumption(ConsumptionNote consumptionNote, Integer people, Integer boxtype,Integer drinknum,String drinkstype) {
		...
		if(consumptionNote.getType() == Const.CON_NOTE_RU_TYPE) {
			...
			if("自制饮品".equals(drinkstype)) {
				consumptionNote.setRemark(drinkstype+ "："+drinknum+"杯，共计"+consumptionNote.getMoney().stripTrailingZeros().toPlainString()+"元。"+consumptionNote.getRemark());
			}else {
